    ...
    with open(options.output, 'w') as fout: 
        for path in paths:
            ...
            for dir_name in dir_list:
                ...
                fq_files = [glob.glob(f'{path}/{sample_name}/{sample_name}_*_{i}{suffix}') for i in ['I1', 'I2', 'R1', 'R2', 'R3'] if len(glob.glob(f'{path}/{sample_name}/{sample_name}_*_{i}{suffix}')) > 0]
                for fqfs in fq_files:
                    print("#" + ";".join(fqfs))
                    for fqf in fqfs:
                        ...
                        print(f'mkdir -p {fqf_folder}\nzcat {fqf} |head -4000000 |gzip - > {fqf_new} &')
            unfq_files = [glob.glob(f'{path}/../Undetermined_*_{i}{suffix}')[0] for i in ['I1', 'I2', 'R1', 'R2', 'R3'] if len(glob.glob(f'{path}/../Undetermined_*_{i}{suffix}')) > 0]
            for fqf in unfq_files:
                basename = os.path.basename(fqf)
                fqf_folder = os.path.join(*[options.outdir, run_name, subfolder, "../"])
                fqf_new = os.path.join(fqf_folder, basename)
                print(f'mkdir -p {fqf_folder}\nzcat {fqf} |head -4000000 |gzip - > {fqf_new} &')
            print("wait")
