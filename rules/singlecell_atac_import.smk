...
rule prep_fastq:
    ...
    output: R1 = "QC/Sample_{sample}/{sample}_R1.fastq.gz", R3 = "QC/Sample_{sample}/{sample}_R3.fastq.gz"
    shell: "ln -s {input.R1} {output.R1} && ln -s {input.R3} {output.R3}"

rule fastqscreen:
    input: R1 = rules.prep_fastq.output.R1, R3 = rules.prep_fastq.output.R3
    output: one = "QC/Sample_{sample}/{sample}_R1_screen.png", two = "QC/Sample_{sample}/{sample}_R3_screen.png", one_txt = "QC/Sample_{sample}/{sample}_R1_screen.txt", two_txt = "QC/Sample_{sample}/{sample}_R3_screen.txt", one_html = "QC/Sample_{sample}/{sample}_R1_screen.html", two_html = "QC/Sample_{sample}/{sample}_R3_screen.html"
    log: logname = "QC/Sample_{sample}/{sample}_fastq_screen.err"
    params: prefix = "QC/Sample_{sample}/"
    shell: "{program.fastq_screen} --outdir {params.prefix} --threads {clusterConfig[fastqscreen][threads]} --subset 5000000 --nohits --conf {program.conf} --aligner bowtie2 {input.R1} {input.R3} 2>{log.logname}"
...
rule kraken:
    ...
    shell: "{program.kraken2} --gzip-compressed --threads {clusterConfig[kraken][threads]} --db {program.kraken2db} --output {params.prefix} --report {output.report} {input.R1} {input.R3} 2> {log.err} && cut -f2,3 {output.result} > {output.krona}"

rule multiqc_qc:
    input: expand("QC/Sample_{sample}/{sample}_R3_screen.png", sample=samples), expand("QC/Sample_{sample}/{sample}.kraken.report.txt", sample=samples)
...
rule summaryFiles:
  	...
  	shell: "{program.python2_7} {program.atac_pythonscripts}/generateSummaryFiles.py"
